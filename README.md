...
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
 ...
 Actions are objects carrying a type, and sometimes payload that is provided to your reducers that intake the action obj and depending on the action type will execute different logic, and modify the store, the aplications storage of the entire applications state, for dynamic rendering. Store is the only state that is used by 95+% of the app(usually excludes forms)
...
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  Thunk allows us to create async calls. This is crucial since we do need to be able to create API calls, which NEED to be loaded async. Thunk allows us to use thunks (functions that return functions[such as my axios calls]) on the basis of promises.
